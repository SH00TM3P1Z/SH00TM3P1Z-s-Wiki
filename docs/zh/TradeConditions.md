...
| 0 | 禁用全局交易条件。仅当物品 json 中 `ItemTradeConditions.Enabled=1` 时可交易 |

> **可交易条件：** 全局 `Enabled` 或物品 `ItemTradeConditions.Enabled` 任一为 1 则按该条件可交易。两者均为 0 则不可交易，显示「Trade Fail」通知。
...
**NeedDeathWishDogtag：** 为 1 时仅 Death Wish 狗牌可用于交易。普通玩家狗牌不可用。为 0 时普通狗牌和 Death Wish 狗牌均可使用。

**RequireSelfKill：** 为 1 时仅自己击杀的狗牌可用于交易。拾取他人击杀的狗牌不可用。为 0 时谁击杀均可使用。

**AllowSuicideDogtag：** 为 1 时自杀获得的狗牌也可用于交易。为 0 时自杀狗牌排除在交易之外。
...
**RequireSelfKill：** 与 PlayerDogtag 相同。为 1 时仅自己击杀的 AI 狗牌，为 0 时谁击杀均可使用。
...
若物品有 `ItemTradeConditions` 且 `Enabled` 为 1，该物品忽略全局设置，仅使用其 PlayerDogtag、AIDogtag 条件。
