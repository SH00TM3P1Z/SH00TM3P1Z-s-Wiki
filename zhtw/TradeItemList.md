...
**NeedDogtagAmount：** 交易该物品一个所需的狗牌数量。DogtagTradeSettings 的 IncludeAttachmentsPrice 為 true 時配件价格会加在此处。

**Attachments：** 该物品的配件列表。配件若也登记在交易物品列表中，IncludeAttachmentsPrice 為 true 時价格会累加。

**ItemTradeConditions：** 无或 Enabled 為 0 時使用全局 TradeConditions。Enabled 為 1 時仅此物品使用不同交易条件。
...
- 无 `ItemTradeConditions` 或 `Enabled: 0` 時使用全局 `TradeConditions`。