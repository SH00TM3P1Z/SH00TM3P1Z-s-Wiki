...
**文件路径:** `$profile:EnhancedDogtag\Config\DogtagTradeSettings.json`
...
| `TimeZone` | string | "UTC" | 时区（例如：KST、UTC） |
...
交易是否可用以**服务器**为准。`IsTradeEnabled()` 返回 false 时交易完全禁止。
...
- **UseDateRange 为 true 时：**
  - 仅 StartDate：当前日期等于或晚于开始日期时可交易
  - 仅 EndDate：当前日期等于或早于结束日期时可交易
  - 两者都有：当前日期在开始与结束日期之间（含）时可交易
  - 两者都空：无日期限制（即使 UseDateRange 为 true）

### TimeZone

- 用于 `StartDate`、`EndDate`、`UseWeekendOnly` 的**日期/星期计算**的时区。
- 将服务器 UTC 时间转换为此时区后进行比较。
...
- **true：** 仅**周六·周日**可交易（以该 TimeZone 为准）。
- **注意：** 使用 `UseWeekendOnly` 必须满足以下全部条件：
  - 将 `UseDateRange` 设为 true
  - 在 `StartDate` 中输入日期
  - 在 `EndDate` 中输入日期
- 缺少任一条件则交易始终禁止。
...
1. UseDateRange 为 false：无日期/周末限制
2. UseDateRange 为 true：检查 `StartDate` 与 `EndDate` 范围
3. UseWeekendOnly 为 true：在上述范围内仅**周六·周日**可交易
...
## IncludeAttachmentsPrice（配件价格详细行为）
...
例如：M4A1 基础 10 个、瞄准镜 2 个、弹匣 1 个时，设为 true 则共需 13 个狗牌。
