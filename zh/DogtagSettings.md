...
此配置文件定义狗牌系统的基本行为（拾取、Death Wish、显示选项等）。与**狗牌交易**分开，控制狗牌物品本身的行为。
...
## GeneralDogtagSettings（一般狗牌设置）

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `AllowDogtagLooting` | bool | true | 允许死亡时拾取狗牌 |
| `DisableSuicideDogtagLooting` | bool | false | 禁用自杀时拾取狗牌 |
| `EnableDogtagRespawnDelay` | bool | false | 启用重生后狗牌拾取延迟 |
| `DogtagRespawnDelaySeconds` | int | 300 | 重生后至可拾取狗牌的延迟时间（秒） |
...
| `KeepKillCountsOnDeath` | bool | true | 死亡时保留击杀数 |

### GeneralDogtagSettings 详细行为

**AllowDogtagLooting：** 设为 false 时无人可从尸体拾取狗牌。设为 true 时满足其他条件即可拾取。

**DisableSuicideDogtagLooting：** 设为 true 时，自杀玩家的狗牌永远无法拾取。设为 false 时自杀狗牌也可拾取（取决于其他条件）。

**EnableDogtagRespawnDelay / DogtagRespawnDelaySeconds：** 仅适用于自杀死亡。设为 true 时，死亡后需经过指定秒数（默认 300 秒，5 分钟）才能拾取该狗牌。设为 false 时自杀狗牌可立即拾取。

**ShowPlayerKills, ShowAIKills：** 切换狗牌物品上显示的玩家击杀数和 AI 击杀数。

**DistanceUnit：** 0=米(m)，1=码(yd)，用于在玩家死亡狗牌信息中显示距离。

**KeepKillCountsOnDeath：** 设为 true 时，死亡时之前生存期间的击杀数会保存到 DogtagLifetime.json 并反映到新狗牌。设为 false 时死亡时击杀数会重置。

---

## DeathWishSettings（Death Wish 设置）

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `DeathWishEnabled` | bool | true | 启用 Death Wish 功能 |
| `ShowLifetimeToDeathWish` | bool | true | 显示至 Death Wish 的生存时间 |
...
**KeepDeathWishOnDeath：** 设为 true 时，死亡重生后仍保留之前的 Death Wish 等级。保存到 DogtagLifetime.json。

**DeathWishRespawnEnabled：** 设为 true 时，根据 Death Wish 等级应用在最后死亡位置重生的概率。使用 DeathWishRespawnSettings 的概率。

---

## NeedDeathWishTimeSettings（Death Wish 时间条件）

所有时间值均为**秒**。（例如：21600=6 小时）

**行为：** 升至各级别需生存相应时间。NeedDeathWishInitialTime 为从普通狗牌变为 Death Wish 所需时间，之后各级别时间为升至下一级所需生存时间。除时间条件外，还需满足击杀数条件（根据 DeathWishConditionType）。

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `NeedDeathWishInitialTime` | int | 21600 | 达到基础 Death Wish 所需生存时间 |
| `NeedDeathWishLevel1Time` | int | 36000 | 达到 1 级所需生存时间 |
| `NeedDeathWishLevel2Time` | int | 54000 | 达到 2 级所需生存时间 |
| `NeedDeathWishLevel3Time` | int | 72000 | 达到 3 级所需生存时间 |
| `NeedDeathWishLevel4Time` | int | 86400 | 达到 4 级所需生存时间 |
| `NeedDeathWishLevel5Time` | int | 129600 | 达到 5 级所需生存时间 |
...
**行为：** 死亡时生成 0 到 99 的随机数，若小于该等级概率则在最后死亡位置重生。例如 DeathWishRespawnChance3 为 50 则 50% 概率重生。100 则始终重生。
...
| 文件 | 用途 |
|------|------|
| **DogtagSettings.json** | 狗牌基本行为（拾取、Death Wish、显示等） |
| **DogtagTradeSettings.json** | 狗牌交易系统（物品交换、条件等） |

使用`狗牌交易商店系统`需将 `DogtagSettings.json` 中的 `DogtagEnabled` 设为 `true`。

[狗牌交易设置 →](DogtagTradeSettings)